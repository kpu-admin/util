package cn.lmx.basic.database.config;
...
/**
 * Mybatis 常用重用拦截器，kpu.database.multiTenantType=任意模式 都需要实例出来
 * <p>
 * 拦截器执行一定是：
...
public abstract class BaseMybatisConfiguration {
    ...
    /**
     * kpu.database.id-type = DEFAULT 或 kpu.database.id-type 未设置时启用。
     */
    @Bean
    @ConditionalOnMissingBean
    ...
    /**
     * kpu.database.id-type = CACHE 时启用。
     */
    @Bean
    @ConditionalOnMissingBean
    ...
    /**
     * kpu.database.id-type = HU_TOOL 时启用。
     */
    @Bean
    @ConditionalOnMissingBean
